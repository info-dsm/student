...
const SearchBar: React.FC<searchBarProps> = ({ url, placeholder }) => {
  ...
  return (
    <form method="GET" action={url + "/search/" + search}>
      <label className={searchStyle.label}>
        <input className={searchStyle.searchInput} type={"text"} placeholder={placeholder} onChange={(e) => setSearch(e.target.value)} />
        <button type={"submit"} className={searchStyle.icon}>
