...
import { getBaseList } from "../../../axios/dist";
...
const WriteNotice = ({ menu }: WriteNoticeProps) => {
  ...
  function handleComplete(data: AddressProps) {
  ...
  const ChangeWork = useCallback(
    (
      ...
      if (props === "isFlexible" || workTime.isFlexible === false) {
  ...
  const ChangeAll = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      const arr = certificateList.list[certificateList.now].small.map(
        (item) => {
      ...
      setCertificateList({
        ...
        list: certificateList.list.map((v, i) => {
          if (i === certificateList.now) {
            return { small: arr, bigClassification: v.bigClassification };
          } else {
            return v;
          }
        }),
  ...
  const ChangeCheck = useCallback(
    (props: "technologe" | "language" | "certificate", i: number) => {
      setCheck({
        ...check,
        [props]: check[props].map((e, index) => (i === index ? !e : e)),
      });
    },
    [check]
  );
        ...
        <_Layout>
          ...
          <_Flex>
            <NumberInput
              ...
              defaultValue={`${number.numberOfEmployee}`}
            />
          </_Flex>
          <_Line />
          <_Title>상세직무</_Title>
          ...
          <_QualifiLayout>
            <_Lay gap={10}>
              ...
              <NumberInput
                ...
                defaultValue={`${workTime.workTimePerWeek}`}
                last={"시"}
              />
              <NumberInput
                text={"일일 근무시간"}
                ...
                defaultValue={`${workTime.workTimePerDay}`}
                last={"시"}
              />
            </_Lay>
            <BigCheck
          ...
          <_FlexWrap>
            <_BlueTextLay>1차전형</_BlueTextLay>
