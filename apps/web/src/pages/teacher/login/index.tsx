...
const TeacherSignIn = () => {
  ...
  type DataProps = "email" | "password";
  const changeInput = (e: any, props: DataProps) => {
    setData({ ...data, [props]: e.target.value });
  };

  const str: {
    name: string;
    additionalElm: string;
    placeholder: string;
    func: () => void;
    check: string;
    key: DataProps;
...
const TitleText = styled.div<{ size: number; weight: number }>`
  text-align: center;
...
const HighlightText = styled.span<{ cursor: string; color: string }>`
